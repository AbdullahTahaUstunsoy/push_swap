...
void ft_join1(int argc, char **argv, int total_size){
    ...
    str = (char *)malloc(sizeof(char) *total_size + 1 + (argc -1 -1)); //fazladan yer ayırmak sorun olur mu burada ? str ye atacağım argümanların boyutu tam doğru hesaplanıyor mu ? //argümanlar birleşirken aralarına boşluk koymak için argc -1 -1 ekledik.
